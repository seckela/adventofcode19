...
function createPath(path, instruction){
	let distance = parseInt(instruction.slice(1));
	let direction = instruction.charAt(0);
	let dirX = direction === "R" ? +1 : direction === "L" ? -1 : 0;
	let dirY = direction === "U" ? +1 : direction === "D" ? -1 : 0;
	let start = path.slice(-1)[0];
	for(let i = 1; i <= distance; i++){
		path.push([start[0] + dirX*i, start[1] + dirY*i]);
	}
}

function distanceToOrigin(location){
...
crossings = p1.filter((point, i) => {
	let p1First = 0;
	let p2First = 0;
	let test1 = p2.some((coord, j) => {
		let test2 = (coord[0] === point[0] && coord[1] === point[1]);
		if(test2){
			p2First = j;
		}
		return test2
	...
	if(test1){
		p1First = i;
		crossSteps.push((p1First + p2First));
	}	
	return test1;
});

answer = Math.min.apply(Math, crossSteps.slice(1));